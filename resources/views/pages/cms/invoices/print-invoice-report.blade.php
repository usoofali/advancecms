...
new #[Layout('layouts.guest')] #[Title('Invoice Report')] class extends Component
{
    ...
    public function mount(Invoice $invoice)
    {
        ...
        if ($this->institution) {
            $adminStaff = \App\Models\Staff::where('institution_id', $this->institution->id)
                ->where('role_id', 2)
...
<div class="p-8 bg-white min-h-screen text-zinc-900 font-sans">
    ...
    <div class="mt-8 pt-6 border-t border-zinc-100 flex justify-between items-end">
        ...
        <div class="text-center w-48 flex flex-col items-center">
            <div class="h-12 flex items-end justify-center mb-1 w-full border-b border-zinc-300">
                @if($adminSignatureUrl)
                    <img src="{{ $adminSignatureUrl }}" alt="Authorized Signature" class="max-h-12 max-w-full object-contain mb-1">
                @endif
            </div>
