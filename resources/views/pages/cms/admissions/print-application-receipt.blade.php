...
new #[Layout('layouts.guest')] #[Title('Print Application Receipt')] class extends Component {
    ...
    public function mount(Receipt $receipt)
    {
        ...
        if (auth()->check() && auth()->user()->hasRole('Student') && auth()->user()->email !== ($receipt->payment?->applicant?->email ?? '')) {
            ...
            <div class="space-y-1">
                <div class="flex border-b border-zinc-100 pb-1">
                    <span class="w-36 text-zinc-500 uppercase text-[10px] font-bold tracking-tight">Received
                        From:</span>
                    <span
                        class="flex-1 uppercase font-bold text-base">{{ $receipt->payment?->applicant?->full_name }}</span>
                </div>
                <div class="flex border-b border-zinc-100 pb-1">
                    <span class="w-36 text-zinc-500 uppercase text-[10px] font-bold tracking-tight">Application
                        No:</span>
                    <span
                        class="flex-1 uppercase font-mono font-bold text-sm">{{ $receipt->payment?->applicant?->application_number }}</span>
                </div>
                <div class="flex border-b border-zinc-100 pb-1">
                    <span class="w-36 text-zinc-500 uppercase text-[10px] font-bold tracking-tight">Program:</span>
                    <span
                        class="flex-1 uppercase font-semibold text-sm">{{ $receipt->payment?->applicant?->program?->name }}</span>
                </div>
                <div class="flex border-b border-zinc-100 pb-1">
                ...
                <div class="flex border-b border-zinc-100 pb-1">
                    <span class="w-36 text-zinc-500 uppercase text-[10px] font-bold tracking-tight">Payment
                        Method:</span>
                    <span
                        class="flex-1 uppercase font-semibold text-zinc-700 text-xs">{{ str_replace('_', ' ', strtoupper($receipt->payment?->payment_method)) }}</span>
                </div>
                <div class="flex border-b border-zinc-100 pb-1">
