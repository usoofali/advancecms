...
use App\Actions\Placements\RejectPlacementAction;
...
new #[Layout('layouts.app')] class extends Component
{
    ...
    public function createPlacement()
    {
        $this->reset([
            ...
            'single_generate_letter', 'single_template_id', 'create_place_mode', 'create_custom_name',
            'create_custom_address', 'create_custom_city', 'create_custom_state',
        ]);
        $this->showModal = true;
    }
...
<div>
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Initiate student cohorts, review posting requests, and issue authorization letters.</p>
        </div>

        <div class="flex flex-wrap items-center gap-2">
            <flux:button wire:click="openBatchModal" variant="filled" icon="rectangle-stack" class="w-full sm:w-auto">Initiate Cohort</flux:button>
            <flux:button wire:click="createPlacement" variant="primary" icon="plus" class="w-full sm:w-auto">New Assignment</flux:button>
        </div>
    </div>

    <flux:card>
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
            <flux:input wire:model.live.debounce.300ms="search" icon="magnifying-glass" placeholder="Search student..." />
                ...
                <flux:table.rows>
                    @forelse($placements as $placement)
                        <flux:table.row>
                            ...
                            <flux:table.cell>
                                <div class="font-medium text-gray-900 dark:text-white">{{ $placement->student->full_name }}</div>
                                <div class="text-xs text-gray-500">{{ $placement->student->matric_number }}</div>
                            </flux:table.cell>
                            <flux:table.cell>
                                <div class="font-medium text-gray-900 dark:text-white">{{ $placement->organization_display_name }}</div>
                            ...
                            <flux:table.cell align="right">
                                <flux:dropdown>
                                    ...
                                    <flux:menu>
                                        <flux:menu.item wire:click="openAssignPlaceModal({{ $placement->id }})" icon="map-pin">
                                        ...
                                        @if($placement->workflow_stage === 'Pending_Request_Approval')
                                            ...
                                            <flux:menu.item wire:click="openRejectModal({{ $placement->id }}, 'organization')" icon="x-mark" variant="danger">
                                        ...
                                        @if($placement->workflow_stage === 'Acceptance_Submitted' || $placement->placementDocuments->isNotEmpty())
                                            <flux:menu.item wire:click="verifyAcceptanceAndPost({{ $placement->id }})" icon="check-badge">
                                                Verify Acceptance & Issue Posting
                                            </flux:menu.item>
                                            <flux:menu.item wire:click="openRejectModal({{ $placement->id }}, 'acceptance')" icon="x-mark" variant="danger">
                                        ...
                                        @if($placement->organization_id && in_array($placement->workflow_stage, ['Request_Approved', 'Acceptance_Submitted', 'Posting_Issued']))
                                            <flux:menu.item wire:click="generateGroupCoverLetterForOrganization({{ $placement->organization_id }})" icon="users">
                                                Generate Group Cover Letter
                                            </flux:menu.item>
                                        @endif

                                        <flux:menu.item wire:click="openGenerateModal({{ $placement->id }})" icon="document-text">
                                        ...
                                        @endphp
                                        @if($generatedDoc)
                                            <flux:menu.item href="{{ route('cms.placements.print', ['doc' => $generatedDoc->document_number]) }}" target="_blank" icon="printer">
                                                View / Print Letter
                                            </flux:menu.item>
                                        @endif

                                        @if($placement->status !== 'Cancelled')
                                            <flux:menu.item wire:click="confirmCancelPlacement({{ $placement->id }})" icon="no-symbol" variant="danger">
                                                Cancel Placement
                                            </flux:menu.item>
                                        @else
                                            <flux:menu.item wire:click="restorePlacement({{ $placement->id }})" icon="arrow-path">
                                                Restore Placement
                                            </flux:menu.item>
                                        @endif
                                    </flux:menu>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="md:col-span-2 space-y-3">
                        <div class="flex space-x-4 border-b border-gray-200 dark:border-gray-700 pb-2">
                            <button type="button" wire:click="$set('create_place_mode', 'directory')" class="text-sm font-medium {{ $create_place_mode === 'directory' ? 'text-primary-600 border-b-2 border-primary-600' : 'text-gray-500' }}">
                                Choose from Directory
                            </button>
                            <button type="button" wire:click="$set('create_place_mode', 'custom')" class="text-sm font-medium {{ $create_place_mode === 'custom' ? 'text-primary-600 border-b-2 border-primary-600' : 'text-gray-500' }}">
                        ...
                        @else
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 bg-gray-50 dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
                                <div class="md:col-span-2">
                                    <flux:input wire:model="create_custom_name" label="Organization Name" placeholder="e.g. General Hospital Abuja" required />
                                </div>
                                <div class="md:col-span-2">
                                    <flux:input wire:model="create_custom_address" label="Street Address" placeholder="e.g. Plot 12, Medical Road" required />
                                </div>
                                <div>
                    ...
                    <flux:input wire:model="academic_session" label="Academic Session" placeholder="2025/2026" required />
    ...
    <flux:modal wire:model="assignPlaceModal" class="w-full max-w-xl">
        <div class="p-6">
            ...
            @if($placementToAssign)
                <p class="text-sm text-gray-500 mb-4">Assigning host facility for <span class="font-semibold text-gray-700 dark:text-gray-300">{{ $placementToAssign->student->full_name }}</span>.</p>
            @endif

            <form wire:submit.prevent="saveAssignedPlace">
                <div class="space-y-4">
                    <div class="flex space-x-4 border-b border-gray-200 dark:border-gray-700 pb-2">
                        <button type="button" wire:click="$set('assign_place_mode', 'directory')" class="text-sm font-medium {{ $assign_place_mode === 'directory' ? 'text-primary-600 border-b-2 border-primary-600' : 'text-gray-500' }}">
                            Directory Organization
                        </button>
                        <button type="button" wire:click="$set('assign_place_mode', 'custom')" class="text-sm font-medium {{ $assign_place_mode === 'custom' ? 'text-primary-600 border-b-2 border-primary-600' : 'text-gray-500' }}">
                            Custom Organization
                        </button>
                    </div>
                    ...
                    @else
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div class="md:col-span-2">
                                <flux:input wire:model="assign_custom_name" label="Organization Name" placeholder="e.g. General Hospital Abuja" required />
                            </div>
                            <div class="md:col-span-2">
                                <flux:input wire:model="assign_custom_address" label="Street Address" placeholder="e.g. Plot 12, Medical Road" required />
                            </div>
                            <div>
                ...
                <div class="mt-6 flex flex-col sm:flex-row sm:justify-end gap-3 sm:space-x-3">
                    <flux:button wire:click="$set('assignPlaceModal', false)" variant="ghost" class="w-full sm:w-auto">Cancel</flux:button>
    ...
    <flux:modal wire:model="batchModal" class="w-full max-w-2xl">
        <div class="p-6">
            ...
            <p class="text-sm text-gray-500 mb-4">Initiate placement requirements for an entire student cohort. Leave Organization unselected to let students choose their place of posting.</p>
            ...
            <form wire:submit.prevent="processBatchGeneration">
                ...
                <div class="mt-6 flex flex-col sm:flex-row sm:justify-end gap-3 sm:space-x-3">
                    <flux:button wire:click="$set('batchModal', false)" variant="ghost" class="w-full sm:w-auto">Cancel</flux:button>
                    <flux:button type="submit" variant="primary" icon="rectangle-stack" class="w-full sm:w-auto">Initiate Cohort</flux:button>
                </div>
            </form>
        </div>
    </flux:modal>
        ...
        <div class="p-6">
            ...
            @if($placementToGenerate)
                ...
                <div>
                    ...
                    <div class="mt-6 flex flex-col sm:flex-row sm:justify-end gap-3 sm:space-x-3">
                        <flux:button wire:click="$set('generateModal', false)" variant="ghost" class="w-full sm:w-auto">Cancel</flux:button>
                        <flux:button wire:click="generateLetter" variant="primary" icon="document-text" class="w-full sm:w-auto">Generate PDF</flux:button>
                    </div>
                </div>
            @endif
    </flux:modal>
        ...
        <div class="p-6">
            ...
            @if($placementToReject)
                <p class="text-sm text-gray-500 mb-4">
                    Please state why this {{ $rejection_type === 'organization' ? 'facility selection' : 'acceptance document' }} is rejected for <span class="font-semibold text-gray-700 dark:text-gray-300">{{ $placementToReject->student->full_name }}</span>. This reason will be shown on the student's portal so they can take appropriate corrective action.
                </p>
            @endif

            <form wire:submit.prevent="processRejection">
                <flux:textarea wire:model="rejection_reason" label="Reason for Rejection / Remarks" placeholder="e.g. Facility is not accredited for SIWES or document scan is blurry..." rows="4" required />
                <flux:error name="rejection_reason" />

                <div class="mt-6 flex flex-col sm:flex-row sm:justify-end gap-3 sm:space-x-3">
                    <flux:button wire:click="$set('rejectModal', false)" variant="ghost" class="w-full sm:w-auto order-2 sm:order-1">Cancel</flux:button>
                    <flux:button type="submit" variant="danger" icon="x-mark" class="w-full sm:w-auto order-1 sm:order-2">Confirm Rejection</flux:button>
                </div>
            </form>
        </div>
    </flux:modal>
    ...
    <flux:modal wire:model="cancelModal" class="w-full max-w-md">
        <div class="p-6">
            ...
            <p class="text-gray-500 mb-6">Are you sure you want to cancel this student's placement? This action will stop their current posting workflow.</p>

            <div class="flex flex-col sm:flex-row sm:justify-end gap-3 sm:space-x-3">
                <flux:button wire:click="$set('cancelModal', false)" variant="ghost" class="w-full sm:w-auto order-2 sm:order-1">No, Keep Placement</flux:button>
                <flux:button wire:click="cancelPlacement" variant="danger" class="w-full sm:w-auto order-1 sm:order-2">Yes, Cancel Placement</flux:button>
            </div>
        </div>
    </flux:modal>
</div>